...
class Modules(object):
    def load(self):
        ...
        for module_path in settings.REQUEST_TRAFFIC_MODULES:
            ...
            traffic_module, traffic_classname = module_path[:dot], \
            module_path[dot + 1:]
    ...
    def table(self, queries):
        return [(module.verbose_name_plural, [module.count(qs)
        for qs in queries]) for module in self.modules]

    def graph(self, days):
        return [{'data':[(mktime(day.timetuple()) * 1000, module.count(qs))
        for day, qs in days], 'label':ugettext(module.verbose_name_plural)}
...
class UniqueVisit(Module):
    def count(self, qs):
        return
        qs.exclude(referer__startswith=settings.REQUEST_BASE_URL).count()
