...
INFO_TABLE_QUERIES = [getattr(Request.objects, query, None)()
for query in INFO_TABLE]
...
class RequestAdmin(admin.ModelAdmin):
    list_display = ('time', 'path', 'priority', 'response', 'method',
    'request_from')
    fieldsets = (
        (_('Request'), {
            'fields': ('method', 'path', 'priority', 'time', 'is_secure',
            'is_ajax')
        }),
        (_('Response'), {
            'fields': ('response',)
    ...
    def request_from(self, obj):
        if obj.user:
            return '<a href="?user__username=%s" title="%s">%s</a>' % \
            (obj.user.username, _('Show only requests from this user.'), \
            obj.user)
        return '<a href="?ip=%s" title="%s">%s</a>' % \
        (obj.ip, _('Show only requests from this IP address.'), obj.ip)
    ...
    def get_urls(self):
        ...
        urls = patterns('',
            url(r'^overview/$', wrap(self.overview), name='%s_%s_overview'
            % info),
        )
    ...
    def overview(self, request):
        ...
        return render_to_response('admin/request/request/overview.html', {
            ...
            'top_paths': set_count(Request.objects.this_month().filter(
            response__lt=400).values_list('path', flat=True))[:10],
            'top_error_paths': set_count(Request.objects.this_month(
            ).filter(response__gte=400).values_list('path', flat=True))[:10],
            'top_referrers': set_count(Request.objects.this_month(
            ).unique_visits().values_list('referer', flat=True))[:10],
            'top_browsers': set_count(Request.objects.this_month().only
            ('user_agent').attr_list('browser'))[:5],
            'top_search_phrases': set_count(Request.objects.this_month(
            ).search().only('referer').attr_list('keywords'))[:10],
