...
QUERYSET_PROXY_METHODS = ('year', 'month', 'week', 'day', 'today', 'this_week',
'this_month', 'this_year', 'unique_visits', 'attr_list', 'search')
...
class RequestQuerySet(models.query.QuerySet):
    ...
    def week(self, year, week):
        try:
            date = datetime.date(*time.strptime(year + '-0-' + week, \
            '%Y-%w-%U')[:3])
        except ValueError:
            return
    ...
    def day(self, year=None, month=None, day=None, month_format='%b',
    day_format='%d', date=None):
        if not date:
            try:
        ...
        return self.filter(time__range=(datetime.datetime.combine(date,
        datetime.time.min), datetime.datetime.combine(date,
        datetime.time.max)))
    ...
    def attr_list(self, name):
        return [getattr(item, name, None) for item in self if hasattr(item,
        name)]

    def search(self):
        return self.filter(referer__contains='google') | self.filter(
        referer__contains='yahoo') | self.filter(referer__contains='bing')
