...
browsers = patterns(('Unknown', {}),
    ...
    (r'Mozilla/(?P<mozilla_version>[-.\w]+) \(compatible; ( ?)MSIE \
    (?P<msie_version>[-.\w]+);( ?)( ?)America Online Browser \
    (?P<version>[-.\w]+);', 'AOL'),
    (r'Camino/(?P<version>[-.\w]+)', 'Camino'),
    (r'Chrome/(?P<version>[-.\w]+)', 'Google Chrome'),
    (r'Firefox(/(?P<version>[-.\w]+)?)', 'Firefox'),
    (r'Mozilla/(?P<mozilla_version>[-.\w]+) \(compatible; \
    ( ?)MSIE (?P<version>[-.\w]+);( ?)( ?)(Win|Mac)', \
    'Internet Explorer'),
    ...
    (r'Minefield(/(?P<version>[-.\w]+)?)', 'Firefox'),
    (r'Shiretoko(/(?P<version>[-.\w]+)?)', 'Firefox'),
    (r'GranParadiso(/(?P<version>[-.\w]+)?)', 'Firefox'),
    (r'Iceweasel(/(?P<version>[-.\w]+)?)', 'Firefox'),
    # RSS Reader
    ...
    (r'Mozilla/(?P<mozilla_version>[-.\w]+) \(compatible; \
    DotBot/(?P<version>[-.\w]+); http://www.dotnetdotcom.org/, \
    crawler@dotnetdotcom.org\)', 'Dotbot'),
    (r'IrssiUrlLog(/(?P<version>[-.\w]+)?)', 'irssi'),
    (r'Linguee Bot \(http://www.linguee.com/bot; bot@linguee.com\)', \
    'Linguee'),
    (r'Sphider', 'Sphider'),
...
engines = patterns(None,
    (r'^https?:\/\/([\.\w]+)?yahoo.*(?:&|\?)p=(?P<keywords>[\+-_\w]+)',
    'Yahoo'),
    (r'^https?:\/\/([\.\w]+)?google.*(?:&|\?)q=(?P<keywords>[\+-_\w]+)',
    'Google'),
    (r'^https?:\/\/([\.\w]+)?bing.*(?:&|\?)q=(?P<keywords>[\+-_\w]+)',
    'Bing'),
)
